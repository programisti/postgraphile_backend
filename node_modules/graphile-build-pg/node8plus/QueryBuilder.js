  ...
  cb) {
    ...
    if (this.parentQueryBuilder) {
      ...
      this.parentQueryBuilder.beforeLock("select", () => {
        ...
        // $FlowFixMe
        this.parentQueryBuilder.select(sql.fragment`json_build_object(
          '__id', ${sql.value(id)}::int
          ${sql.join(Object.keys(allRequirements).map(key => sql.fragment`, ${sql.literal(key)}::text, ${allRequirements[key]}`), ", ")}
          )`, "__live");
      });
    } else if (cb) {
      cb(checkerGenerator);
